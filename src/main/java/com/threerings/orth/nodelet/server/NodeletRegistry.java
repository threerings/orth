...
/**
 * Implement nodelet materialization, assuming the hosting of nodelets is governed by a DSet in
...
public abstract class NodeletRegistry
{
     ...
     * @see OrthNodeObject
     */
    public NodeletRegistry (String dsetName)
    {
        _dsetName = dsetName;
    }

    public void materialize (final ClientObject caller, final Nodelet nodelet,
            final ResultListener<HostedNodelet> listener)
    {
        if (materializeExisting(nodelet, listener)) {
             return;
        }
     ...
     */
    protected boolean materializeExisting (Nodelet nodelet, ResultListener<HostedNodelet> listener)
    {
